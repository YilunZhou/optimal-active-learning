...
def main(model_seeds=[0, 1, 2, 3, 4], data_seed=0, batchsize=25, max_epoch=100, patience=20, tot_acq=300):
    ...
    with shelve.open('statistics/perf_curves.shv') as curves:
        for i, m_to in enumerate(model_seeds):
            for j, m_from in enumerate(model_seeds):
                spec_trans = f'{m_from} {m_to} {data_seed} {batchsize} {max_epoch} {patience} {tot_acq}'
                spec_native = f'{m_to} {m_to} {data_seed} {batchsize} {max_epoch} {patience} {tot_acq}'
    ...
    plt.title('Object Classification', fontsize=13)
    plt.savefig('../figures/object_classification/seed_transfer.pdf', bbox_inches='tight')
