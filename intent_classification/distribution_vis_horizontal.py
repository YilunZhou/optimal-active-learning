...
def main(model='lstm', model_seed=0, domain='alarm', data_seed=0,
    ...
    gs = GridSpec(ncols=15, nrows=1, width_ratios=[10, 0.7, 10, 0.7, 10, 0.7, 10, 1, 10, 0.7, 10, 0.7, 10, 0.7, 10], wspace=0.05)
    ...
    plt.title('Optimal')
    plt.xticks(np.linspace(N_warmstart, N_warmstart + tot_acq, 5)[:-1])
    ...
    fig.add_subplot(gs[0, 8 + 1])
    plot_ref_meter(label_ref_cdf)

    fig.add_subplot(gs[0, 8 + 2])
    ...
    fig.add_subplot(gs[0, 8 + 3])
    plot_ref_meter(label_ref_cdf)

    fig.add_subplot(gs[0, 8 + 4])
    ...
    fig.add_subplot(gs[0, 8 + 5])
    plot_ref_meter(label_ref_cdf)

    fig.add_subplot(gs[0, 8 + 6])
