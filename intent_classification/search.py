...
class Runner():
    ...
    def evaluate_order(self, order):
        ...
        assert len(curve) == int(self.args.tot_acq / self.args.batchsize) + 1
    ...
    def run(self):
        assert not self.runned, 'run() is supposed to be called only once'
        self.runned = True
...
def main(model, model_seed, domain='alarm', data_seed=0, batchsize=20, max_epoch=100, patience=20, tot_acq=160,
         workers_per_gpu=1, use_gpus='all', anneal_factor=0.1, num_sa_samples=40000,
         num_greedy_samples=5000, log_dir='logs'):
    args = Namespace(model=model, model_seed=model_seed, domain=domain, data_seed=data_seed,
                     batchsize=batchsize, max_epoch=max_epoch, patience=patience,
                     tot_acq=tot_acq, workers_per_gpu=workers_per_gpu, use_gpus=use_gpus,
                     anneal_factor=anneal_factor, num_sa_samples=num_sa_samples,
                     num_greedy_samples=num_greedy_samples, log_dir=log_dir)
    print(args)
    runner = Runner(args)
    runner.run()
...
def main_cli():
    ...
    ################## required args ##################
    parser.add_argument('--model', type=str, choices=['lstm', 'cnn', 'aoe', 'roberta'],
        help=ht.model)
    parser.add_argument('--model-seed', type=int, choices=[0, 1, 2, 3, 4],
        help=ht.model_seed)
    ################## required args ##################
