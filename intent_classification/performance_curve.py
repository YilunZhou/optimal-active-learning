...
def plot_curves(order, xs, evaluation_set, model, search_model_seed, eval_model_seed, domain, data_seed,
                batchsize, max_epoch, patience, tot_acq, use_gpus, workers_per_gpu, baselines):
    legends = []
    spec = f'{model} {search_model_seed} {eval_model_seed} {domain} {data_seed} {batchsize} {max_epoch} {patience} {tot_acq}'
    with shelve.open(abs_path('statistics/perf_curves.shv')) as curves:
        if spec in curves and curves[spec]['order'] == order:
            ...
            eval_args = Namespace(model=model, model_seed=eval_model_seed, domain=domain, data_seed=data_seed,
                                batchsize=batchsize, max_epoch=max_epoch, patience=patience, evaluation_set=evaluation_set,
    ...
    for name, display_name, color in baselines:
        try:
            curve = load_baseline(name, evaluation_set, model, eval_model_seed, domain, data_seed,
...
def main(model, search_model_seed, eval_model_seed=None, domain='alarm', data_seed=0, tot_acq=160,
         batchsize=20, max_epoch=100, patience=20, use_gpus='all', workers_per_gpu=1, log_dir='logs'):
    if eval_model_seed is None:
        eval_model_seed = search_model_seed

    N_warmstart = 40
    optimal_order, optimal_quality, _ = load_optimal(log_dir, model, search_model_seed, domain, data_seed,
    ...
    plot_args = [model, search_model_seed, eval_model_seed, domain, data_seed,
                 batchsize, max_epoch, patience, tot_acq, use_gpus, workers_per_gpu, baselines]
    ...
    plt.tight_layout()
    fn = f'../figures/intent_classification/perf_curves/{model}_s{search_model_seed}_e{eval_model_seed}.pdf'
